...
void TMAG5273::configOperatingMode(tmag5273_operating_mode mode) { operatingMode = mode; }

void TMAG5273::configReadMode(tmag5273_read_mode mode) { readMode = mode; }
...
uint8_t TMAG5273::readSensorArray(float* data_ptr) {
    if (operatingMode == TMAG5273_OPERATING_MODE_STANDBY) {
        writeRegisterGeneral(DEVICE_ID | 0x80, 0x00);
        delayMicroseconds(estiConversionTime);
    }
...
void TMAG5273::initAll(void)  {
    writeRegisterGeneral(DEVICE_CONFIG_1, (REG_DEVICE_CONFIG_1_DEFAULT & 0x80) | magTempcoMode | convAvgMode | readMode);
    writeRegisterGeneral(DEVICE_CONFIG_2, (REG_DEVICE_CONFIG_2_DEFAULT & 0xEC) | lplnMode | operatingMode);
    writeRegisterGeneral(SENSOR_CONFIG_1, REG_SENSOR_CONFIG_1_DEFAULT);
    writeRegisterGeneral(SENSOR_CONFIG_2, (REG_SENSOR_CONFIG_2_DEFAULT & 0xFC) | magRange);
    ...
    writeRegisterGeneral(INT_CONFIG_1, REG_INT_CONFIG_1_DEFAULT);
...
uint8_t TMAG5273::readMagneticField(float* Bx, float* By, float* Bz, float* T) {
    if (operatingMode == TMAG5273_OPERATING_MODE_STANDBY) {
        writeRegister(DEVICE_ID | 0x80, 0x00);
        delayMicroseconds(estiConversionTime);
    }
    return _readMagneticField(Bx, By, Bz, T);
}

uint8_t TMAG5273::_readMagneticField(float* Bx, float* By, float* Bz, float* T)
{
    ...
    int16_t xData = (i2c_dev->read() << 8) | i2c_dev->read();
    int16_t yData = (i2c_dev->read() << 8) | i2c_dev->read();
    int16_t zData = (i2c_dev->read() << 8) | i2c_dev->read();
